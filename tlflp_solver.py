...
Improved TLFLP QUBO Solver with:
  ...
  6. Post-processing repair heuristic for near-feasible solutions
...
@_dc
class InstanceData:
    I:    List[int] = _field(default_factory=list)
    C:    List[int] = _field(default_factory=list)
    R:    List[int] = _field(default_factory=list)
    a_ij: Dict[Tuple[int,int], float] = _field(default_factory=dict)
    b_jr: Dict[Tuple[int,int], float] = _field(default_factory=dict)
    p_i:  Dict[int, float] = _field(default_factory=dict)
    q_j:  Dict[int, float] = _field(default_factory=dict)
    c_j:  Dict[int, float] = _field(default_factory=dict)
    d_r:  Dict[int, float] = _field(default_factory=dict)
    N_c:  Dict[int, List[int]] = _field(default_factory=dict)
    T_r:  Dict[int, List[int]] = _field(default_factory=dict)
...
class InstanceParser:
    ...
    def parse_matrix_line(self, line):
        p = line.strip().split()
        if len(p) == 3:
            return int(p[0]), int(p[1]), float(p[2])
        raise ValueError(f"Bad matrix line: {line}")

    def parse_dict_line(self, line, section_name=None):
    ...
    def parse_mapping_line(self, line):
        if ':' in line:
            kp, vp = line.split(':', 1)
            return int(kp.strip()), self.parse_list(vp)
    ...
    def parse_content(self, content):
        ...
        for line in lines:
            ...
            if not line or line.startswith('###'):
                continue
            if line.startswith('I (') or line.startswith('I:'):   sec = 'I';   continue
            elif line.startswith('C (') or line.startswith('C:'): sec = 'C';   continue
            elif line.startswith('R (') or line.startswith('R:'): sec = 'R';   continue
            elif line.startswith('a_ij'): sec = 'a_ij'; continue
            elif line.startswith('b_jr'): sec = 'b_jr'; continue
            elif line.startswith('p_i'):  sec = 'p_i';  continue
            elif line.startswith('q_j'):  sec = 'q_j';  continue
            elif line.startswith('c_j'):  sec = 'c_j';  continue
            elif line.startswith('d_r'):  sec = 'd_r';  continue
            elif line.startswith('N_c'):  sec = 'N_c';  continue
            elif line.startswith('T_r'):  sec = 'T_r';  continue
            try:
                if sec == 'I':   self.data.I = self.parse_list(line)
                elif sec == 'C': self.data.C = self.parse_list(line)
                elif sec == 'R': self.data.R = self.parse_list(line)
                elif sec == 'a_ij':
                    i,j,v = self.parse_matrix_line(line); self.data.a_ij[(i,j)] = v
                elif sec == 'b_jr':
                    j,r,v = self.parse_matrix_line(line); self.data.b_jr[(j,r)] = v
                elif sec == 'p_i':
                    for i,v in self.parse_dict_line(line,'p_i'): self.data.p_i[i] = v
                elif sec == 'q_j':
                    for j,v in self.parse_dict_line(line,'q_j'): self.data.q_j[j] = v
                elif sec == 'c_j':
                    for j,v in self.parse_dict_line(line,'c_j'): self.data.c_j[j] = v
                elif sec == 'd_r':
                    for r,v in self.parse_dict_line(line,'d_r'): self.data.d_r[r] = v
                elif sec == 'N_c':
                    j,pl = self.parse_mapping_line(line); self.data.N_c[j] = pl
                elif sec == 'T_r':
                    r,cs = self.parse_mapping_line(line); self.data.T_r[r] = cs
            except Exception as e:
                print(f"  [WARN] Cannot parse line in {sec}: {line}  ({e})")
        return self.data
...
# ──────────────────────────────────────────────────────────────────────────────
# Post-processing: repair near-feasible solutions
# ──────────────────────────────────────────────────────────────────────────────

def repair_solution(data: InstanceData, x: dict, y: dict, z: dict
                    ) -> Tuple[dict, dict, dict]:
    """
    Greedily repair constraint violations:
      v1: ensure each RDC is served by exactly one CDC
      v3: ensure z_j=1 wherever CDC j has supply assigned
    """
    x = dict(x); y = dict(y); z = dict(z)

    # Fix v1: each RDC served exactly once
    for r in data.R:
        assigned = [(j,r) for j in data.T_r[r] if y.get((j,r),0) == 1]
        if len(assigned) == 0:
            # Assign cheapest CDC
            best_j = min(data.T_r[r], key=lambda j: data.b_jr.get((j,r), 1e9))
            y[(best_j, r)] = 1
        elif len(assigned) > 1:
            # Keep cheapest, remove rest
            best_jr = min(assigned, key=lambda jr: data.b_jr.get(jr, 1e9))
            for jr in assigned:
                if jr != best_jr:
                    y[jr] = 0

    # Fix z: activate CDCs that have y_{j,r}=1
    for j in data.C:
        has_rdc = any(y.get((j,r),0)==1 for r in data.R if (j,r) in data.b_jr)
        if has_rdc and z.get(j,0) == 0:
            z[j] = 1
        elif not has_rdc and z.get(j,0) == 1:
            # optionally deactivate to save cost – only if no supply assigned
            has_supply = any(x.get((i,j),0)==1 for i in data.N_c[j])
            if not has_supply:
                z[j] = 0

    # Fix x: for active CDCs, ensure at least one plant assigned if needed
    for j in data.C:
        if z.get(j,0) == 1:
            has_supply = any(x.get((i,j),0)==1 for i in data.N_c[j])
            if not has_supply and data.N_c[j]:
                best_i = min(data.N_c[j], key=lambda i: data.a_ij.get((i,j),1e9))
                x[(best_i,j)] = 1
    ...
    for t in range(1, num_iters + 1):
        ...
        for sample_dict, energy in sample_list:
            ...
            if total_v == 0:
                ...
                # Try repair
                xr, yr, zr = repair_solution(data, x, y, z)
                v1r, v2r, v3r = count_violations(data, xr, yr, zr)
                if v1r + v2r + v3r == 0:
                    obj = original_objective(data, xr, yr, zr)
                    sol = QUBOSolution(xr, yr, zr, obj, energy, feasible=True)
                    elite_pool.append(sol)
                    if best is None or obj < best.obj:
                        best = sol

            # Track best violation tuple for APL
            vtuple = (v1, v2, v3)
